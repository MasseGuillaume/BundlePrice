...
"""
  ...
  def free = {
    ...
    bundle(cart) ==== Set(CartState(Map('b -> 1), Map('a -> 1)))
  }
  def fixedPrice = {
  ...
  def orSelectionPrice = {
    ...
    bundle(cart) ==== Set(
      ...
      CartState(Map('a -> 2, 'b -> 2),Map('c -> 2),5), 
      CartState(Map('a -> 2),Map('b -> 1, 'c -> 1),5), 
      CartState(Map('b -> 2, 'c -> 3),Map('a -> 2),5), 
      CartState(Map('b -> 2),Map('a -> 1, 'c -> 1),5), 
      CartState(Map('c -> 3),Map('a -> 1, 'b -> 1),5)
    )
  }
}